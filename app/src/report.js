...
export async function createTimesheet(filename) {
	...
	for (const project of jsonData) {
		...
		var projectWorksheet = XLSX.utils.aoa_to_sheet([
			[{v: "Task", s: styleBlue}, {v:"Subtask", s: styleBlue},  {v:"Time(Hr)", s: styleBlue}], // header
		]);
		// set taskOrigin
		var taskOrigin = {
			r: 1,
		...
		XLSX.utils.sheet_add_aoa(
			projectWorksheet,
			[[{v:"TOTAL:", s:styleYellow},{v:"", s:styleYellow},{v:totalTime, s:styleYellow}]],
			{ origin: taskOrigin });
	...
	await XLSXStyle.default.writeFile(wb, homedir() + "/Downloads/" + filename + ".xlsx");
